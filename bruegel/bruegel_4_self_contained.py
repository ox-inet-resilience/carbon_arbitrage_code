...
def run(year, branch):
    ...
    for public_funding, text in public_funding_dict.items():
        ...
        data_gov_cost[public_funding] = pd.read_csv(
            file_prefix_gov_cost + f"{public_funding}.csv"
        )
    ...
    with open(f"plots/bruegel/bruegel_4_combined_{year_branch}.csv", "w") as f:
        ...
        for name in public_funding_dict.values():
            row += [name] + [""] * (
                len(columns_gov_cost) + len(columns_net_benefit) - 1
            )
        writer.writerow(row)
        ...
        for country in EMDEs:
            ...
            for public_funding in public_funding_dict.keys():
                ...
                row_gov_cost = (
                    df_gov_cost[df_gov_cost["Country name"] == country]
                    .iloc[0]
                    .tolist()[1:]
                )
...
def run_without_gov_cost(year, branch):
    year_branch = f"{year}_{branch}"
    file_prefix = f"plots/bruegel/bruegel_4_{year_branch}_public_funding_"
    data = {}
    for public_funding, text in public_funding_dict.items():
    ...
    with open(f"plots/bruegel/bruegel_4_combinednogovcost_{year_branch}.csv", "w") as f:
        writer = csv.writer(f)
        columns_net_benefit = data[1].columns.tolist()[1:]
        row = [""]
        for name in public_funding_dict.values():
            row += [name] + [""] * (len(columns_net_benefit) - 1)
        writer.writerow(row)

        row = ["Recipient countries"] + columns_net_benefit * 4
        writer.writerow(row)
...
for branch in branches:
    ...
    for year in years:
        print(branch, year)
        # run(year, branch)
        run_without_gov_cost(year, branch)
