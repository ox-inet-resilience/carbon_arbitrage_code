...
def cached_get_export_fraction(country):
...
def modify_based_on_coal_export(element):
    ...
    for country, v in element.items():
        ...
        if math.isclose(v, 0.0):
            ...
            fraction = cached_get_export_fraction(country)
        ...
        for cc, vv in element.items():
            if cc == country:
                # Skip self
                continue
            total_import += cached_get_import_fraction(cc, country) * vv
        new[country] = domestic + total_import
    # Now we do non-masterdata countries, which only do
    # imports
    for country in non_masterdata_alpha2:
        total_import = sum(
            cached_get_import_fraction(cc, country) * vv for cc, vv in element.items()
        )
